       ...
       if(document.querySelector('card-totalizadores-component') != null && document.querySelector('.lb-total.tp-geral.width-auto span:not(.lb-percent)').innerText === "Casos novos") {
           ...
           fetch('https://us-central1-covid-5ce43.cloudfunctions.net/getStaticCovidData')
            .then(async response => {
               ...
               const totalObitosEl = document.getElementById('totalObitosId');
               const totalCasosEl = document.getElementById('totalCasosId');
              
               totalObitosEl.innerText = new Intl.NumberFormat('pt-BR').format(covidData.totalObitos);
               totalCasosEl.innerText = new Intl.NumberFormat('pt-BR').format(covidData.totalCasos);

               totalObitosEl.innerHTML = totalObitosEl.innerHTML + "<br><div style='font-size: 14px; color: var(--ion-color-grey-2); margin-top: 15px'>Acumulados</div>";
               totalCasosEl.innerHTML = totalCasosEl.innerHTML + "<br><div style='font-size: 14px; color: var(--ion-color-grey-2); margin-top: 15px'>Acumulados</div>";
            });
